...
export async function joinGame(gameId: string, userId: string, name: string, avatarUrl: string) {
  ...
  const ps = [...g.players.filter((p: any) => p.uid !== userId), { uid: userId, name, score: 0, streak: 0, completedCategories: [], avatarUrl }];
...
export async function recordAnswer(gameId: string, questionId: string, userId: string, answer: GameAnswer) {
  await supabase.rpc('record_game_answer', {
    p_game_id: gameId,
...
export const subscribeToMessages = (game_id: string, callback: (messages: any[]) => void) => {
  const channel = supabase
    ...
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'game_messages', filter: `game_id=eq.${game_id}` }, () => {
...
export async function sendMessage(game_id: string, user_id: string, content: string) {
  await supabase.from('game_messages').insert({ game_id, user_id, content, timestamp: new Date().toISOString() });
}

async function loadMessages(game_id: string) {
  const { data } = await supabase.from('game_messages').select('*, profiles(display_name, photo_url)').eq('game_id', game_id).order('timestamp', { ascending: true }).limit(50);
...
export async function updatePlayerActivity(gameId: string, userId: string, isResume = false) {
  const { data: g } = await supabase.from('games').select('players').eq('id', gameId).single();
  if (!g) return;
  const now = Date.now();
  const ps = g.players.map((p: any) => p.uid === userId ? { ...p, lastActive: now, lastResumedAt: isResume ? now : p.lastResumedAt } : p);
  await updateGame(gameId, { players: ps });
}

export async function abandonGame(gameId: string) {
  await updateGame(gameId, { status: 'abandoned', current_question_id: null, current_question_category: null, current_question_started_at: null });
}

export async function setActiveGameQuestion(gameId: string, cat: string, qId: string, idx: number, start: number) {
  await updateGame(gameId, { current_question_id: qId, current_question_category: cat, current_question_index: idx, current_question_started_at: start });
}

export async function clearActiveGameQuestion(gameId: string) {
  await updateGame(gameId, { current_question_id: null, current_question_category: null, current_question_started_at: null });
}
